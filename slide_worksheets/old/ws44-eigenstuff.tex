...
\begin{frame}{\fn}
	\begin{exercise}
		...
		Prove that $c\vec x$ is also an eigenvector of $A$ corresponding to $\lambda$ for all scalars $c\neq 0$.
	\end{exercise}
	\begin{exercise}
		Let $\vec x_1$ and $\vec x_2$ be eigenvectors of $A$ corresponding to some fixed eigenvalue $\lambda$.  
		
		Prove that $\vec x_1+\vec x_2$ is also an eigenvector of $A$ corresponding to $\lambda$.
	\end{exercise}
\end{frame}
\begin{frame}{\fn}
...
\begin{frame}{\fn}
	\begin{exercise}\label{first}
		...
		\begin{enumerate}[label=(\alph*)]
			...
			\only<1>{\begin{center}(Go to the next page to see what you should get.)\end{center}}\pause
...
\begin{frame}{\fn}
	\begin{block}{\textbf{Brain Break.}}
		Where are you from?  (Yes also remind your table-mates of your name.)
		\begin{center}
...
\begin{frame}{\fn}
	\begin{question}[Finding Eigenvalues]
		...
		\begin{enumerate}[label=(\alph*)]
			...
			\item What does it mean for the matrix $\lambda I-A$ to have a non-zero solution to the homogeneous system $(\lambda I-A)\vec x=\vec 0$?\pause
			\item It means that $\lambda I-A$ is singular (non-invertible).  What is a quick way to determine whether a matrix is singular?\pause
